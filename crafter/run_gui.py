    ...
    while running:
        ...
        if args.stats:
            ...
            stats_rect = pygame.Rect(
                args.window[0], 0, stats_panel_width, args.window[1]
            )
            ...
            # Episode counter
            episode_text = stats_font.render(
                f"Episode: {env._episode}", True, (200, 200, 255)
            )
            ...
            # Health
            health_color = (
                (100, 255, 100) if env._player.health > 5 else (255, 100, 100)
            )
            health_text = stats_font.render(
                f"Health: {env._player.health}", True, health_color
            )
            ...
            if reward_display_timer > 0:
                ...
                reward_text = stats_font.render(
                    f"Reward: +{last_reward:.1f}", True, reward_color
                )
                step_text = stats_font_small.render(
                    f"@ Step {last_reward_step}", True, (150, 150, 150)
                )
            ...
            ach_text = stats_font.render(
                f"Achievements:\n{len(achievements)}/{total_achievements}",
                True,
                (255, 255, 100),
            )
            ...
            if recent_achievements:
                ...
                for ach in recent_achievements:
                    ach_display = stats_font_small.render(
                        f"✓ {ach}", True, (100, 255, 100)
                    )
        ...
        if args.profile and frames_since_profile >= args.profile_frames:
            ...
            if rss_kb >= 0:
                print(
                    f"[PROFILE] fps={fps:.1f} rss={rss_kb/1024:.1f}MB step={env._step}"
                )
            else:
